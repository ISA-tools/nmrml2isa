...
class UserMetaLoader(object):
    ...
    CATEGORIZED_MAP = collections.OrderedDict([
        ...
        ("Investigation", collections.OrderedDict([
        ...
        ("Contacts", collections.OrderedDict([
            ...
            ("Study Contacts Adress", [
                ("study_contacts", "adress"), True,
            ]),

            ("Study Contacts Email", [
                ("study_contacts", "email"), True,
            ]),

            ("Study Contacts Phone", [
                ("study_contacts", "phone"), True,
            ]),

            ("Study Contacts Fax", [
                ("study_contacts", "fax"), True,
            ]),
        ...
        for row in sheet.iter_rows():
            ...
            if not more_than_one:
                ...
                for i,path_node in enumerate(true_name[:-1]):
                    item_to_set = item_to_set.setdefault(path_node, {true_name[i+1]: {}})
        ...
        for mv_key in (v[0][0] for v in six.itervalues(self.MAP) if v[1]):
            try:
                for value in self.usermeta[mv_key]:
                    empty = not any(v for k,v in six.iteritems(value)
